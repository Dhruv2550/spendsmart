...
import { TrendingUp, BarChart3, Brain, Calendar, Clock, AlertTriangle, CheckCircle, TrendingDown, Activity, FileText, Download, Printer, Bot, X } from "lucide-react";
...
const AnalyticsPage = ({ monthlySavings }: AnalyticsPageProps) => {
  ...
  const optimisticMonthChange = (newMonth: string) => {
    ...
    addToast(`Switched to ${monthName} analytics`, 'info', 2000);
    setBackgroundLoading(true);
    
    setTimeout(() => setBackgroundLoading(false), 1500);
  };

  const optimisticReportGeneration = async (reportType: 'pdf' | 'csv') => {
    ...
    if (reportType === 'pdf') {
      ...
      addToast(`Generating ${monthName} PDF report...`, 'info', 3000);
      
      try {
        await exportReportAsPDF();
        addToast('PDF report generated successfully', 'success', 3000);
      } catch (error) {
        addToast('Failed to generate PDF report', 'error', 5000);
      } finally {
        setGeneratingReport(false);
      }
    } else {
      addToast(`Exporting ${monthName} data to CSV...`, 'info', 2000);
      
      try {
        exportDataAsCSV();
        addToast('CSV export completed', 'success', 3000);
      } catch (error) {
        addToast('Failed to export CSV data', 'error', 4000);
      }
    }
  ...
  const exportReportAsPDF = async () => {
    ...
    if (printWindow) {
      ...
      return new Promise<void>((resolve, reject) => {
        ...
        printWindow.onerror = () => reject(new Error('Failed to load report'));
      });
    } else {
        ...
        <div className="container mx-auto px-4 py-8">
          ...
          <div className="flex justify-between items-center mb-8">
            ...
            <Button
              ...
              className="flex items-center gap-2 bg-gradient-to-r from-primary to-primary/90 hover:from-primary/90 hover:to-primary transition-all duration-200"
            >
          ...
          {selectedMonth !== currentMonth && (
            <div className="mb-6 text-sm text-muted-foreground bg-blue-50 border border-blue-200 rounded-lg p-3 transition-all duration-300">
              ...
              <Card className="border-0 shadow-md bg-gradient-to-br from-card to-card/95 transition-all duration-300 hover:shadow-lg">
                ...
                <CardContent>
                  <div className="flex items-center justify-between mb-4">
                    <div>
                      <div className={`text-4xl font-bold mb-2 transition-all duration-300 ${getHealthScoreColor(healthScore).split(' ')[0]}`}>
                        {healthScore}/100
                      </div>
                      <div className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-300 ${getHealthScoreColor(healthScore)}`}>
                    ...
                    <div className="relative w-24 h-24">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        ...
                        <circle
                          ...
                          className={`transition-all duration-1000 ${healthScore >= 80 ? 'text-green-500' : healthScore >= 60 ? 'text-blue-500' : healthScore >= 40 ? 'text-yellow-500' : 'text-red-500'}`}
                        />
                      </svg>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="border-0 shadow-md bg-gradient-to-br from-card to-card/95 transition-all duration-300 hover:shadow-lg">
                  ...
                  <CardContent className="space-y-3">
                    {categoryTrends.slice(0, 3).map(trend => (
                      <div key={trend.category} className={`p-3 rounded-lg transition-all duration-300 hover:shadow-sm ${
                        trend.trend === 'up' ? 'bg-red-50 border border-red-200 hover:bg-red-100' : 
                        trend.trend === 'down' ? 'bg-green-50 border border-green-200 hover:bg-green-100' : 
                ...
                <Card className="border-0 shadow-md bg-gradient-to-br from-card to-card/95 transition-all duration-300 hover:shadow-lg">
                  ...
                  <CardContent className="space-y-4">
                    <div className="text-center">
                      <div className="text-2xl font-bold text-orange-600 transition-all duration-300">${spendingVelocity.dailyBurnRate.toFixed(0)}</div>
                      ...
                      return (
                        <div key={pattern.dayOfWeek} className="space-y-2">
                          <div className="flex justify-between items-center text-sm">
                            <span className={`font-medium transition-all duration-200 ${isWeekend ? 'text-purple-600' : ''}`}>
                              {pattern.dayOfWeek}
                            </span>
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    {Array.from({ length: new Date(parseInt(selectedMonth.split('-')[0]), parseInt(selectedMonth.split('-')[1]), 0).getDate() }, (_, i) => {
                      ...
                      return (
                        <div
                          key={day}
                          className={`aspect-square rounded text-xs flex items-center justify-center font-medium transition-all duration-300 hover:scale-110 cursor-pointer ${
                            amount > 0 
                              ? intensity > 0.7 ? 'bg-red-500 text-white hover:bg-red-600' :
                                intensity > 0.4 ? 'bg-red-300 text-red-900 hover:bg-red-400' :
                                intensity > 0.1 ? 'bg-red-100 text-red-700 hover:bg-red-200' :
                                'bg-red-50 text-red-600 hover:bg-red-100'
                              : 'bg-gray-100 text-gray-400 hover:bg-gray-200'
                          }`}
                          title={`${day}: $${amount.toFixed(0)}`}
                          onClick={() => amount > 0 && addToast(`${day}: $${amount.toFixed(0)} spent`, 'info', 2000)}
                        >
                          {day}
                        </div>
                      );
                    ...
                    <div className="h-80">
                      <svg width="100%" height="100%" viewBox="0 0 800 320" className="border rounded">
                        ...
                        {(() => {
                          ...
                          return (
                            <>
                              <path d={incomePath} fill="none" stroke="#10b981" strokeWidth="3" className="transition-all duration-1000" />
                              <path d={expensePath} fill="none" stroke="#ef4444" strokeWidth="3" className="transition-all duration-1000" />
                              <path d={savingsPath} fill="none" stroke="#3b82f6" strokeWidth="3" className="transition-all duration-1000" />
                              
                              {monthlyTrendData.map((d, i) => (
                                <g key={i}>
                                  <circle cx={40 + i * xStep} cy={280 - (d.totalIncome * yScale) + yOffset} r="5" fill="#10b981" className="transition-all duration-300 hover:r-7 cursor-pointer" />
                                  <circle cx={40 + i * xStep} cy={280 - (d.totalExpenses * yScale) + yOffset} r="5" fill="#ef4444" className="transition-all duration-300 hover:r-7 cursor-pointer" />
                                  <circle cx={40 + i * xStep} cy={280 - (d.netSavings * yScale) + yOffset} r="5" fill="#3b82f6" className="transition-all duration-300 hover:r-7 cursor-pointer" />
                                </g>
                              ))}
                        ...
                        <div className="space-y-2">
                          {topCategories.slice(0, 4).map((category, index) => {
                            const totalSpent = transactions
                              ...
                              .reduce((sum, t) => sum + t.amount, 0);
                            return (
                              <div key={category} className="flex items-center gap-2 transition-all duration-300 hover:bg-gray-50 p-1 rounded">
                                <div 
              ...
              <DialogContent className="max-w-md">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex gap-3">
                    <Button
                      onClick={() => optimisticReportGeneration('pdf')}
                    ...
                    <Button
                      variant="outline"
                      onClick={() => optimisticReportGeneration('csv')}
